...
/// toggle (enable/disable), and delete. Surfaces every backend error in a
/// SnackBar so the real API behaviour is visible.
...
class _FeatureFlagsScreenState extends State<FeatureFlagsScreen> {
  ...
  DartStreamSession get _ds => widget.session.dsSession!;
  ...
  String _readable(Object e) {
    if (e is DartStreamApiException) return 'HTTP ${e.statusCode}: ${e.body}';
    return e.toString();
  }
      ...
      child: ListTile(
        ...
        trailing: busy
            ...
            : Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Switch(value: enabled, onChanged: (_) => _toggle(flag)),
                  IconButton(
                    tooltip: 'Delete',
